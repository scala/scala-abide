...
object CompilerProvider extends CompilerProvider

trait TreeProvider {

  val global : Global = CompilerProvider.compiler
  ...
  def fromFile(fileName: String) : global.Tree = {
    val file = new BatchSourceFile(fileName, scala.io.Source.fromFile(fileName).mkString("\n"))
    fromSource(file)
  }
