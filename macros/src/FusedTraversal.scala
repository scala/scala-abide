...
object FusedTraversal {
  def apply(u : scala.reflect.api.Universe)
           (traversals : Traversal { val universe : u.type }*) :
           FusedTraversal { val universe : u.type } = {
...
trait FusedTraversal {
  ...
  private[traversal] type TraversalType = Traversal { val universe : FusedTraversal.this.universe.type }
  ...
  def fuse(that : TraversalType) :
    FusedTraversal { val universe : FusedTraversal.this.universe.type } = new FusedTraversal {
      val universe : FusedTraversal.this.universe.type = FusedTraversal.this.universe
      val traversals = FusedTraversal.this.traversals :+ that
      val emptyStates = FusedTraversal.this.emptyStates :+ that.emptyState
    }

  def fuse(that : FusedTraversal { val universe : FusedTraversal.this.universe.type }) :
    FusedTraversal { val universe : FusedTraversal.this.universe.type } = new FusedTraversal {
      val universe : FusedTraversal.this.universe.type = FusedTraversal.this.universe
   ...
   */
  class FusedResult private[traversal](val mapping : Map[TraversalType, _]) {
